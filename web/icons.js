// icons.js — flat, modern, stroke-based inline SVG icon set (lucide-style).
//
// Every icon is drawn on a 24x24 viewBox with fill="none", stroke="currentColor",
// stroke-width 2 and round caps/joins, so icons inherit the surrounding text
// color and scale crisply. No external requests are made for icons.
//
//   import { icon, ICON_NAMES, faviconUrl } from '/icons.js';
//   el.innerHTML = icon('vault', { size: 20, cls: 'sidebar-icon' });
...
const PATHS = {
  ...
  identity: '<rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="12" cy="10" r="3"/><path d="M7 21v-2a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2"/>',
  ...
  eyeOff: '<path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 11 8 11 8a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 1 12s4 8 11 8a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" x2="22" y1="2" y2="22"/>',
...
// Placeholder rendered for any unknown icon name (never throws).
 ...
 * @param {string} name  one of ICON_NAMES (unknown names render a circle placeholder)
 * @param {{size?: number, cls?: string}} opts
 * @returns {string} SVG markup
 */
export function icon(name, opts = {}) {
  const { size = 16, cls = '' } = opts || {};
  const body = (typeof name === 'string' && PATHS[name]) || FALLBACK;
  const className = ('icon ' + (cls || '')).trim();
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" ` +
    `fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ` +
    `class="${className}" aria-hidden="true">${body}</svg>`;
}

/**
 * Google S2 favicon URL for a site (the only remote icon source, opt-in per call).
 * Accepts full URLs or bare hosts ('github.com'); 'https://' is assumed when
 * the scheme is missing. Returns null when no hostname can be parsed.
 */
export function faviconUrl(siteUrl, size = 32) {
